...
#Function to make random class bots move in a random circle. Returns integer
def randoCircle(initPos, basePos):
...
def ActRobot(robot):
    ...
    robot.setSignal(sig)
    if(Initsig[0] == "R"):
        return randoCircle(location_data["self_coor"], (int(Initsig[1:3]) + 1, int(Initsig[3:5]) - 1))
    if(sig[0] == "r"):
        return randoCircle(location_data["self_coor"], (20, 20))
    elif(sig[0] == "a"):
